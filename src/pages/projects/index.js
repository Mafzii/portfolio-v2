    ...
    return (
        <BaseLayout additional="overflow-hidden">
            ...
            <div className="flex flex-col h-full">
                <p className="type-kicker mb-3 flex-shrink-0">Selected work</p>
                <h1 className="type-display text-5xl sm:text-6xl mb-6 flex-shrink-0 text-neutral-50">projects</h1>
                <ul className="flex-1 overflow-y-scroll pr-2">
                    {projects.map(project => (
                        <li key={project.slug} className="m-0">
                            <div className="block py-6 px-6 rounded-[1.5rem] bg-neutral-800/70 shadow-md ring-1 ring-white/8 hover:bg-neutral-800/90 transition-colors duration-200 mb-3">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                                    <span className="type-nav text-3xl text-neutral-100">{project.name}</span>
                                    <span className="type-meta">{new Date(project.date).toLocaleDateString('en-GB')}</span>
                                </div>
                                <p className="type-body mb-4 text-neutral-300">{project.description}</p>
                                <div className="flex flex-wrap gap-2 mb-2">
                                    {project.tech.map(tech => (
                                        <span key={tech} className="type-meta bg-neutral-900/60 text-neutral-200 px-2.5 py-1.5 rounded-full normal-case tracking-[0.08em]">{tech}</span>
                                    ))}
                                </div>
